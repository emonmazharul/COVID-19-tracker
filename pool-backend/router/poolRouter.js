...
router.post('/pool', async (req,res) => {
	const {creatorEmail,creatorName,poolQuestion,poolOptions,expireAt=24} = req.body;
	const pool = new Pool({
		poolQuestion,
		creatorEmail,
		creatorName,
	...
	try {
		await pool.save();
		res.status(201).send({
			success:'Successfully created your pool. Copy the link below and share it among voters',
			link:`${process.env.PORT}/pool/${pool._id}`,
		});
	} catch (e) {
		console.log(e);
		res.status(400).send({
...
router.patch('/pool/:id', voterAuth, async (req,res) => {
	...
	try {
		...
		const totalVote = pool.poolOptions.map(option => option.vote).reduce((acc,item) => acc+item);
