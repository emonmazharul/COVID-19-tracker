	...
	try {
		...
		if(pool.poolEnded){
			return res.status(200).send({
				pool:pool,
				message:'This Pool has ended.You can\'t vote anymore.We will send the result to the creator and delete this pool',
			})
		}
		...
		console.log(e);
		res.status(404).send({error:"Don't find any ongoing vote on this link."});
	}
}
...
async function voterAuth(req,res,next) {
	...
	try {
		...
		const hasVoted = pool.voters.find(voter => voter.voterIP === req.ip || voter.voterEmail === voterEmail);
		if(hasVoted) throw new Error('You already have voted');
		// if givis an invalid option we give a error back.
		const options = pool.poolOptions.findIndex(option => option.optionName === chooseOption)
